...
public:
    ...
    void TestPeriodicCubeWithGhosts() throw (Exception)
    {
        ...
        std::string output_directory = "Packing/Random_Dynamic_Sorting_Square/Noise_0";
        ...
        double depth_space = sqrt(0.5); //Magic number for z-spaceing... 
        ...
        for (unsigned k=0; k<(depth + 1 ); k++) //+1 puts a layer of ghosts on the bottom
        {
            ...
            {    
                for (unsigned i=0; i<width; i++)
                {
                    ...
                    x_coordinate = (double) ( i + 0.5*(k%2)+ rand_spread*(2.0*RandomNumberGenerator::Instance()->ranf()-1.0 ))*width_space;
                    y_coordinate = (double) ( j + 0.5*(k%2)+ rand_spread*(2.0*RandomNumberGenerator::Instance()->ranf()-1.0 ))*height_space;
        ...
        MAKE_PTR_ARGS(UniformCellKiller3dWithGhostNodes, random_cell_death, (&cell_population, 1.0, periodic_width ,  0 - 1, periodic_height + 1,  0 - 1 , real_node_indices.size()));
